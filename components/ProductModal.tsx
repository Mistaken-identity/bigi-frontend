...
import { XMarkIcon, StarIcon, SparklesIcon, CheckIcon, XCircleIcon } from '../icons';
      ...
      const getSummary = async () => {
        if (reviews.length > 1) {
          ...
          if (response.ok && response.data) {
            ...
            console.error("Failed to get review summary:", response.error);
            // Non-critical error, just log it and don't show a summary.
          }
                ...
                <div className="overflow-y-auto p-6 md:p-8 flex-grow">
                    ...
                    <div className="mt-12 pt-8 border-t">
                        ...
                        {(isSummaryLoading || summary) && (
                            <div className="bg-gray-800 text-white p-6 rounded-lg mb-8 border border-orange-400 shadow-lg">
                                <h4 className="flex items-center text-xl font-semibold mb-4 text-orange-400">
                                    <SparklesIcon className="w-6 h-6 mr-2" />
                                    AI-Generated Summary
                                </h4>
                                {isSummaryLoading ? (
                                    <p className="animate-pulse text-gray-300">✨ AI is analyzing reviews...</p>
                                ) : summary && (
                                                ...
                                                <ul className="space-y-2">
                                                    {summary.cons.map((con, i) => (
                                                        <li key={`con-${i}`} className="flex items-start">
                                                            ...
                                                            <span className="text-gray-200 text-sm">{con}</span>
                                                        </li>
                                                    ))}
                                                </ul>
                                            </div>
